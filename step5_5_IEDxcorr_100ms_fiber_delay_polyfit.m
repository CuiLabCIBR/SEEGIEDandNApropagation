...
for nS = 1:length(subG)
        ...
        [R_fibNxcD(nS), P_fibNxcD(nS)] = corr(fibN, IEDxcD, "type", "Spearman");
        ...
        for order = 1:6 
            ...
            [R_fibNxcD_rED(nS, order), P_fibNxcD_rED(nS, order)] = corr(fibNres, IEDxcDres, "type", "Spearman");
        end
end
parfor nIf = 1:1000
        disp(nIf);
        % 1. Raw correlation between fiber count and IED cross-correlation
        [R_fibNxcD_as(nIf), P_fibNxcD_as(nIf)] = iforest_spearman(fibNas, IEDxcDas);
        % 2. Distance-corrected correlation
        [R_fibNxcD_rED_as(nIf, :), P_fibNxcD_rED_as(nIf, :)] ...
            = iforest_spearman_regressConfound(fibNas, IEDxcDas, EDas);
end
% Save results
saveFolder = 'step5_IEDxcorr_polyfit';
saveName = 'IED6d0_XC100ms_polyfit_delay_fibN';
save(fullfile(saveFolder, saveName), ...
    'EDas', 'IEDxcDas', 'fibNas', ...
