...
for nS = 1:length(IEDcascade)
    ...
    for nRun = 1:length(IEDcascade_eachsub)
        IEDcascade_i = IEDcascade_eachsub{nRun};
        for nC = 1:length(IEDcascade_i)
            N = N + 1;
            TWchanList{N, 1} = IEDcascade_i(nC).unit;
        end
    end
...
savename = 'op4_IEDtw_3motif_probability_observedVSnull.mat';
