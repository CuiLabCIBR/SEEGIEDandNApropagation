% Repeated cross-validation analysis of IED cross-correlation delay prediction.
%
% For each subject, the script:
% 1) loads subject-level cross-correlation and structural connectivity measures,
...
% - xcFC : cross-correlation strength (Fisher z-transformed)
...
for nS = 1:length(subG)
        ...
        IEDxcDelay_fitting_nofib{nS} = stats;
end
%% Save all subject-level CV results
saveFolder = 'step5_IEDxcorr_TD-6d0_IT-100ms_fitting';
saveName = 'op5_delay_ED_FC_noFib_fitting.mat';
save(fullfile(saveFolder, saveName), 'IEDxcDelay_fitting_nofib');
